...
import { getJobById, getJobs } from "../../src/api/jobs";

describe("getJobs(title, location, isFullTime, limit, skip)", () => {
  ...
  it("should return a number of elements specified by the limit argument", async () => {
    const limit = 40;
    const elements = await getJobs("", "", false, limit);
  ...
  it("should return an empty array if provided with a skip argument that exceeds the boundary of the array", async () => {
    const skip = 999999;
    const elements = await getJobs("", "", false, 20, skip);
  ...
  it("should skip a number of elements specified by the skip argument", async () => {
    const skip = 20;
    const firstTwentyElements = await getJobs("", "", false, 20);
    const nextSkippedElements = await getJobs("", "", false, 20, skip);
